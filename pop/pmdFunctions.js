...
function pmdRandom(ext) {
    const timestamp = Date.now();
    const random = Math.floor(Math.random() * 10000);
    return `${timestamp}_${random}${ext}`;
}

async function pmdFancy(text) {
    return new Promise((resolve, reject) => {
...
const pmdBuffer = async (url, options = {}) => {
    try {
        ...
        console.error("pmdBuffer Error:", err);
        return err;
    }
};

const pmdJson = async (url, options = {}) => {
    try {
        const res = await axios({
            method: 'GET',
            url: url,
        ...
        console.error("pmdJson Error:", err);
        return err;
    }
};

const latestWaVersion = async () => {
    const get = await pmdJson("https://web.whatsapp.com/check-update?version=1&platform=web");
...
module.exports = { dBinary, eBinary, dBase, eBase, runtime, sleep, pmdFancy, stickerToImage, toAudio, toVideo, toPtt, formatVideo, formatAudio, monospace, formatBytes, sleep, pmdBuffer, pmdJson, latestWaVersion, pmdRandom, isUrl,pmdStore, isNumber, loadSession, verifyJidState };

...
const { createContext } = require("./pmdHelpers");
...
const PopkidApiKey = '_0u5aff45,_0l1876s8qc';
const PopkidTechApi = 'https://api.giftedtech.co.ke';
async function PopkidAutoReact(emoji, ms,Popkid) {
  try {
    const react = {
      react: {
        text: emoji,
    ...
    await Popkid.sendMessage(ms.key.remoteJid, react);
  } catch (error) {
...
const PopkidAntiLink = async (Popkid, message, antiLink) => {
    try {
        ...
        await Popkid.sendMessage(from, { delete: message.key });

        if (antiLink === 'kick') {
            await Popkid.groupParticipantsUpdate(from, [sender], 'remove');
...
const PopkidAutoBio = async (Popkid) => {
                try {
                    ...
                    await Popkid.updateProfileStatus(bioText);
                } catch (error) {
                }
            };
...
const availableApis = [
    `${PopkidTechApi}/api/ai/ai?apikey=${PopkidApiKey}&q=`,
    `${PopkidTechApi}/api/ai/mistral?apikey=${PopkidApiKey}&q=`,
    `${PopkidTechApi}/api/ai/meta-llama?apikey=${PopkidApiKey}&q=`
];
...
const identityPatterns = [
                ...
                /whos\s*u\??/i,
                /whos?\s*this\??/i,
                /you\s*called\s*popkid/i,
                /are\s*you\s*popkid/i,
                /are\s*u\s*popkid/i,
                /u\s*popkid\??/i,
...
async function getAIResponse(query) {
    if (isIdentityQuestion(query)) {
        return 'I am an Interactive Ai Assistant Chat Bot, created by Popkid Tech!';
    }
...
function PopkidChatBot(Popkid, chatBot, chatBotMode, createContext, createContext2, googleTTS) {
...
const PopkidPresence = async (Popkid, jid) => {
    try {
        const isGroup = jid.endsWith('@g.us');
        ...
        await Popkid.sendPresenceUpdate(whatsappPresence, jid);
...
const PopkidAnticall = async (json, Popkid) => {
   for (const id of json) {
      if (id.status === 'offer') {
         if (antiCall === "true" || antiCall === "decline") {
            let msg = await Popkid.sendMessage(id.from, {
               text: `${antiCallMsg}`,
               mentions: [id.from],
            });
            await Popkid.rejectCall(id.id, id.from);
            ...
            await Popkid.rejectCall(id.id, id.from); 
            await Popkid.updateBlockStatus(id.from, "block");
         }
      }
   }
};
...
const PopkidAntiDelete = async (Popkid, deletedMsg, key, deleter, sender, botOwnerJid, deleterPushName, senderPushName) => {
    ...
    if (isJidGroup(key.remoteJid)) {
        try {
            chatInfo = `💬 Group Chat: ${(await Popkid.groupMetadata(key.remoteJid)).subject}`;
        } catch (error) {
            ...
            promises.push((async () => {
                try {
                    ...
                    if (deletedMsg.message?.conversation || deletedMsg.message?.extendedTextMessage?.text) {
                        ...
                        await Popkid.sendMessage(key.remoteJid, {
                            text: `${baseAlert}\n\n📝 *Content:* ${text}`,
                            mentions: [deleter],
                            ...context
                        });
            ...
            promises.push((async () => {
                try {
                    ...
                    if (deletedMsg.message?.conversation || deletedMsg.message?.extendedTextMessage?.text) {
                        ...
                        await Popkid.sendMessage(botOwnerJid, { 
...
module.exports = { logger, emojis, PopkidAutoReact, PopkidTechApi, PopkidApiKey, PopkidAntiLink, PopkidAutoBio, PopkidChatBot, PopkidAntiDelete, PopkidAnticall, PopkidPresence };

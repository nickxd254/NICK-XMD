const { pmd, commands } = require("../pop");
    ...
    exec(q, options, async (err, stdout, stderr) => {
      try {
        ...
        if (zipPath && fsA.existsSync(zipPath)) {
          await handleZipFile(from, Popkid, mek, react, zipPath);
          return;
        }

        if (stdout) {
          if (stdout.length > 10000) {
            await handleLargeOutput(from, Popkid, mek, react, stdout);
          } else {
            await react("✅");
            await reply(stdout);
          }
...
async function handleZipFile(from, Popkid, mek, react, zipPath) {
  try {
    await react("📦");
...
async function handleLargeOutput(from, Popkid, mek, react, stdout) {
  await react("📤");
  let extension = '.txt';
...
pmd({
  ...
  const { 
    ...
    botVersion, groupAdmins, participants, ownerNumber, ownerName, botName, popkidRepo, 
    getMediaBuffer, getFileContentType, bufferToStream, 
    uploadToPixhost, uploadToImgBB, uploadToGithubCdn, 
    uploadToPopkidCdn, uploadToPasteboard, uploadToCatbox, 
    newsletterUrl, newsletterJid, PopkidTechApi, PopkidApiKey, 
    botPrefix, pmdBuffer, pmdJson, formatAudio, formatVideo, timeZone 
  } = conText;
...
pmd({
  ...
  try {
    ...
    if (target.startsWith('@') && !target.includes('@lid')) {
      ...
      try {
        const lid = await Popkid.getLidFromJid(target);
        if (lid) {
          target = lid;
...
pmd({
  ...
  try {
    ...
    tempFilePath = await Popkid.downloadAndSaveMediaMessage(quotedImg, 'temp_media');
  ...
  try {
    if (quoted) {
      if (isGroup && !targetUser.endsWith('@s.whatsapp.net')) {
        try {
          const jid = await Popkid.getJidFromLid(targetUser);
      ...
      try {
        profilePictureUrl = await Popkid.profilePictureUrl(targetUser, "image");
      } catch (error) {
      ...
      try {
        const statusData = await Popkid.fetchStatus(targetUser);
...
pmd({
  ...
  try {
    ...
    const tempFilePath = await Popkid.downloadAndSaveMediaMessage(quotedImg, 'temp_media');
    const imageBuffer = await fs.readFile(tempFilePath);
    try {
      await Popkid.updateProfilePicture(Popkid.user.id, { url: tempFilePath });
...
pmd({
  ...
  try {
    if (quoted) {
      try {
        profilePictureUrl = await Popkid.profilePictureUrl(quotedUser, "image");
...
pmd({
  ...
  try {
    let profilePictureUrl;
    try {
      profilePictureUrl = await Popkid.profilePictureUrl(from, "image");
    } catch (error) {
      await react("❌");
...
pmd({ 
  ...
  description: "Reveal View Once Media"
    ...
    try {
        ...
        tempFilePath = await Popkid.downloadAndSaveMediaMessage(mediaMessage, 'temp_media');
        ...
        await Popkid.sendMessage(from, msg);
      await react("✅");
    } catch (e) {
...
pmd({ 
  ...
  description: "Reveal View Once Media"
    ...
    try {
        ...
        await Popkid.sendMessage(Popkid.user.id, msg);
      await react("✅");
    } catch (e) {
        console.error("Error in vv command:", e);
        reply(`Error: ${e.message}`);
